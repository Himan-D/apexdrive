...
/**
 * Actuator Operational State Machine (Aligned with CiA 402 / Industrial Motion Standards)
 */
enum class DriveState : uint8_t {
    ...
    FAULT_OVERCURRENT = 8,
    FAULT_BROWNOUT = 9,
    FAULT_OVERVOLTAGE = 10,
    FAULT_OVERTEMP = 11,
    FAULT_WATCHDOG_TIMEOUT = 12,
    FAULT_ENCODER_ERROR = 13,
    SAFE_TORQUE_OFF = 14
};
...
[[nodiscard]] constexpr std::string_view StateToString(DriveState state) noexcept {
    switch (state) {
        ...
        case DriveState::FAULT_OVERVOLTAGE:      return "FAULT_OVERVOLTAGE";
        ...
        case DriveState::SAFE_TORQUE_OFF:        return "SAFE_TORQUE_OFF";
        default:                                 return "UNKNOWN";
    }
}

/**
 * Hardware Telemetry & State Structs
 */
...
struct MotorProfile {
    float phase_resistance_ohm{0.182f};   // Phase Resistance R (Ohms)
    float phase_inductance_h{0.000118f};  // Phase Inductance L (Henries)
    ...
    float encoder_offset_rad{0.0f};       // Calibrated electrical angle offset
};
